...
df_import <- read.csv(here("_tdt/data/clean/dev.csv"), header = TRUE) %>%
  filter(!is.na(fate)) # drop lost bugs cuz they f shit up?
```
...
dfs_tidy <- list(
  wide = df_import %>%
    ...
    ## calc expt exit times from rec/return/exit
    mutate(tt.exitfromenter = dt.exit - dt.enter,
           tt.exitfromrecover = dt.exit - dt.recover,
           tt.exitfromreturn = dt.exit - dt.return) %>%

    ## recode km status with how package expects it
    ## 1 = death, 0 = censored
      ## TODO: wip -- doesnt match up w fates...
    # mutate(status = case_when(dh.exit < max ~ 1,
    #                           TRUE ~ 0)) %>%
    select(-max)
)
```
