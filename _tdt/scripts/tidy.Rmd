...
dfs_tidy[1:2] <- dfs_tidy[1:2] %>% # for the non-sim data only
  lapply(., \(x) {
    x %>% mutate(
      ...
      # total time at 40C
      tt.timeat40 = case_when(trt == 40 ~ tt.exit.fromenter,
                              trt.duration != 0 & trt.enter == 40 ~ 
                                tt.exit.fromreturn + trt.exposure
                              # trt.duration != 0 & trt.enter == 40 & cohort == "A" ~
                              #   tt.exit.fromreturn + 48,
                              # trt.duration != 0 & trt.enter == 40 & cohort != "A" ~
                              #   tt.exit.fromreturn + 24
      ),
