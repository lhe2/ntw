...
dates/times:
- parse culled better: either culled before expected death (pmc) or after the expt bugs were done (culled)
...
df_filtered <- df_raw %>%
  ...
  mutate(fate = case_when(!is.na(date.pmc) ~ "2",
                          !is.na(date.culled) | 
                            (trt.duration == 0 & !is.na(date.4th)) ~ "1", # catch straggler ctrls
                          !is.na(date.died) ~ "0")
         )
