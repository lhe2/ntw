...
rerun in entirety when need to update the data sheets!!
...
fix `instar.exit` (not calc'd right on from gsheets)
...
add `trt.exposure` to separate cohort A + B/C bugs
- cohort A = 48h initial exposure before recovery; B/C = 24h initial exposure
...
df_filtered <- df_raw %>%
  ...
  mutate(across(c("date.recover", "date.return"), 
                ~ case_when(trt.duration == 0 ~ date.enter, 
                            TRUE ~ .)),
         across(c("time.recover", "time.return"), 
                ~ case_when(trt.duration == 0 ~ time.enter, 
                            TRUE ~ .))) %>%
  ...
  mutate(date.pmc = case_when(!is.na(date.culled) & 
         ...
         date.pmd = case_when(trt %in% c(19, 26, 33) & 
                                !is.na(date.died) & is.na(date.4th) ~ date.died),
  ...
  ## add in `trt.exposure`
  mutate(trt.exposure = case_when(#trt.enter == 40 & trt.duration == 0 ~ "0",
                                  cohort == "A" & trt.duration != 0 ~ "48",
                                  cohort != "A" & trt.duration != 0 ~ "24"),
         .after = "trt.enter")
```
...
df_cleaned <- df_filtered %>%
  ...
  # create date-time objs
  mutate(dh.hatch = paste(date.hatch, time.hatch),
  ...
  mutate(across(starts_with("date."), ymd),
         ...
         across(starts_with("dh."),
                ~ (yday(.)*24 + hour(.) + minute(.)/60)) # decimal hrs
         ) %>%
  rename_with(., ~paste0("j", .), starts_with("date.")) %>% # days
  mutate(across(c(trt, trt.enter, trt.recover, trt.duration, trt.exposure, #fate,
                  id, instar.exit, starts_with(c("mass", "jdate"))), as.numeric)) %>%
  
  # fix missing recover/return times (typos in spreadsheet?)
  mutate(
    dh.recover = case_when(
      is.na(dh.recover) & is.na(dh.return) & (dh.exit > dh.enter + trt.exposure) ~ 
        dh.enter + trt.exposure,
      TRUE ~ dh.recover),
    jdate.recover = case_when(
      is.na(jdate.recover) & !is.na(dh.recover) & cohort == "A" ~ jdate.enter + 2,
      is.na(jdate.recover) & !is.na(dh.recover) & cohort != "A" ~ jdate.enter + 1,
      TRUE ~ jdate.recover),
    dh.return = case_when(
      is.na(dh.return) & (dh.exit > dh.recover + trt.duration) ~ 
        dh.recover + trt.duration,
      TRUE ~ dh.return),
    jdate.return = case_when(
      is.na(jdate.return) & !is.na(dh.return) & trt.duration == 24 ~ jdate.recover + 1,
      is.na(jdate.return) & !is.na(dh.return) & trt.duration == 48 ~ jdate.recover + 2,
      TRUE ~ jdate.return)
    )
```
...
```{r}
# clean up
rm(df_raw, df_filtered, df_cleaned, today)
```
