...
trttime_lstats %>%
  ...
  geom_text(aes(label = n, #labs.surv, 
                vjust = -0.9),
            size = 3.5, lineheight = 0.9,
            check_overlap = TRUE, show.legend = FALSE) +
  ...
  labs(title = "avg instar duration (days)",
       y = "treatment", x = "days since 3rd",
       color = "treatment",
       #caption = "n (% surv)"
       caption = "excludes pmd and 6ths; n incls. culled"
       )
```
...
```{r}
trttime_ctrlstats %>%
  mutate(cohort = factor(cohort, c("B", "A"))) %>%
  ...
  scale_x_continuous(breaks = seq(0, 18, by = 2)) +
  theme_bw() +
  #scale_color_manual(values = c("#2166ac", "#92c5de", "#f4a582")) +
  labs(title = "avg instar times (days)",
       y = "cohort", x = "days since 3rd",
       color = "treatment",
       caption = "ctrl trts only"
